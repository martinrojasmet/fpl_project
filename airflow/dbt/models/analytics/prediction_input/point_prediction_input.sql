with player_upcoming_games_combinations as (
    select
        ...
        nug.fpl_game_id,  
        nug.datetime,
        EXTRACT(YEAR FROM nug.datetime) AS year,
        EXTRACT(MONTH FROM nug.datetime) AS month,
        EXTRACT(DAY FROM nug.datetime) AS day,
        EXTRACT(HOUR FROM nug.datetime) AS hour,   
        nug.team_id,
